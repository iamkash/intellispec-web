...
export abstract class BaseGadget extends BaseComponent {
  ...
  ): GadgetHeaderContent | undefined;

  // OPTIONAL: Gadgets can provide footer content
  renderFooter?(
    props: any,
    context?: GadgetContext
  ): GadgetFooterContent | undefined;
  ...
  render(props: any): React.ReactNode {
    ...
    console.warn(
      `Direct render() called on gadget ${this.metadata.id}. Use renderStructured() instead.`
    );
  ...
  private renderBodyWithDataFetching(
    config: GadgetConfig,
    context?: GadgetContext
  ): React.ReactNode {
    if (!config.dataUrl) {
      ...
      React.useEffect(() => {
        const fetchData = async () => {
          try {
            ...
            // Resolve placeholders in fetchUrl using BaseGadget utility
            const resolvedFetchUrl = BaseGadget.resolvePlaceholders(
              fetchUrl,
              gadgetContext
            );

            // Check if there are still unresolved placeholders
            if (/\{[^}]+\}/.test(resolvedFetchUrl)) {
              console.error(
                `[${name}] Cannot fetch data with unresolved placeholders:`,
                resolvedFetchUrl
              );
            ...
            //console.log(`[${name}] Fetching data from:`, resolvedFetchUrl);

            // Temporary simple fetch for debugging
            //console.log(`[DEBUG] Trying simple fetch for: ${resolvedFetchUrl}`);
            let usedUrl = resolvedFetchUrl;
            ...
            let simpleResponse = await BaseGadget.makeAuthenticatedFetch(
              usedUrl
            );
            ...
            if (
              ...
              for (const candidate of candidates) {
                try {
                  const resp = await BaseGadget.makeAuthenticatedFetch(
                    candidate
                  );
            ...
            if (!simpleResponse.ok) {
              throw new Error(
                `HTTP ${simpleResponse.status}: ${simpleResponse.statusText}`
              );
            }
            if (!contentType.includes("application/json")) {
              ...
              throw new Error(
                `Expected JSON but received '${contentType}'. Response starts with: ${snippet}`
              );
            }
            ...
            let processedData = Array.isArray(jsonData)
              ? jsonData
              : jsonData && Array.isArray((jsonData as any).items)
              ? (jsonData as any).items
              : jsonData;
            ...
            try {
              ...
              if (requestedPrefix && Array.isArray(processedData)) {
                processedData = processedData.filter(
                  (it: any) =>
                    typeof it?.id === "string" &&
                    it.id.startsWith(requestedPrefix!)
                );
              }
            } catch {
            ...
            if (
              ...
              try {
                ...
                if (localItems.length > 0) {
                  ...
                  const refetch = await BaseGadget.makeAuthenticatedFetch(
                    usedUrl
                  );
                  const ct2 = refetch.headers.get("content-type") || "";
                  if (refetch.ok && ct2.includes("application/json")) {
                    const jd2 = await refetch.json();
                    processedData = Array.isArray(jd2)
                      ? jd2
                      : jd2 && Array.isArray((jd2 as any).items)
                      ? (jd2 as any).items
                      : jd2;
                    if (requestedPrefix && Array.isArray(processedData)) {
                      processedData = processedData.filter(
        ...
        fetchData();
      }, [fetchUrl, fetchPath, name, gadgetContext]);
  ...
  static getTenantIdForRequest(url: string, options?: RequestInit): string {
    ...
    try {
      ...
      if (tenantId) {
        ...
        console.warn(
          "⚠️ Using default-tenant for debugging (no tenant ID in user data)"
        );
        return "default-tenant";
      }
    } catch (error) {
  ...
  /**
   * Resolve URL placeholders like {id}, {documentId}, etc.
   *
   * This method collects values from multiple sources in order of precedence:
   * 1. URL query parameters
   * 2. Workspace session cache
   * 3. Global portfolio selection cache
   * 4. Context params (highest priority)
   *
   * @param url - URL string that may contain placeholders like {id}
   * @param context - Optional gadget context with params
   * @returns Resolved URL with placeholders replaced
   */
  static resolvePlaceholders(url: string, context?: GadgetContext): string {
    if (!url) return url;
    ...
    if (typeof window !== "undefined") {
      try {
        const urlParams = new URLSearchParams(window.location.search);
        const workspaceKey = urlParams.get("workspace");
        if (workspaceKey) {
          const cached = window.sessionStorage.getItem(
            `workspace:${workspaceKey}`
          );
          if (cached) {
            Object.assign(sources, JSON.parse(cached));
          }
        }
      } catch {
        // Ignore errors
    ...
    // Check global portfolio selection
    if (typeof window !== "undefined") {
      try {
        const globalSelection = window.sessionStorage.getItem(
          "portfolio:last-selection"
        );
        if (globalSelection) {
          const parsed = JSON.parse(globalSelection);
          Object.assign(sources, parsed.params || {});
        }
      } catch {
        // Ignore errors
      }
    ...
    // Get context params (highest priority)
    if (context && (context as any)?.params) {
      Object.assign(sources, (context as any).params);
    }

    // Normalize id/documentId
    if (sources.documentId && !sources.id) {
      sources.id = sources.documentId;
    }
    if (sources.id && !sources.documentId) {
      sources.documentId = sources.id;
    }

    // Replace placeholders
    const resolved = url.replace(/\{([^}]+)\}/g, (_, token) => {
      const value = sources[token];
    ...
    if (/\{[^}]+\}/.test(resolved)) {
      console.warn("[BaseGadget] Unresolved placeholders in URL:", {
        original: url,
        resolved,
        availableSources: Object.keys(sources),
        unresolvedTokens: resolved.match(/\{[^}]+\}/g),
      });
    }
   ...
   */
  static async makeAuthenticatedFetch(
    url: string,
    options: RequestInit = {}
  ): Promise<Response> {
    ...
    return config.widgets.map((widgetConfig) => {
      ...
      if (!widget) {
        return React.createElement(
          "div",
          { key: widgetConfig.id },
          `Widget not found: ${widgetConfig.id}`
        );
      }
  ...
  protected applyLayout(
    ...
    switch (layoutType) {
      case "grid":
        return React.createElement(
          "div",
          { className: "gadget-grid-layout", style: layoutProps },
          ...widgets
        );
      case "flex":
        return React.createElement(
          "div",
          { className: "gadget-flex-layout", style: layoutProps },
          ...widgets
        );
      case "absolute":
        return React.createElement(
          "div",
          { className: "gadget-absolute-layout", style: layoutProps },
          ...widgets
        );
      default:
        return React.createElement(
          "div",
          { className: "gadget-custom-layout", style: layoutProps },
          ...widgets
        );
    }
  }
...
export class GadgetFactory {
  static createGadget(
    ...
    if (!validation.isValid) {
      throw new Error(
        `Invalid gadget configuration: ${validation.errors.join(", ")}`
      );
    }
  ...
  static getGadgetsForWidgets(widgetTypes: string[]): GadgetMetadata[] {
    return gadgetRegistry
      .findCompatibleGadgets(widgetTypes)
      .map((gadget: BaseGadget) => gadget.metadata);
  }
}
