...
import { CalculatorOutlined, ClearOutlined, FilePdfOutlined } from '@ant-design/icons';
...
export class DynamicCalculatorGadget extends BaseGadget {
  ...
  renderBody(props: any): React.ReactNode {
    ...
    return <SimpleCalculatorComponent config={config as SimpleCalculatorConfig} />;
  }
...
const CalculatorHeader = React.memo<{
  ...
  const isValidIcon = IconCmp && (typeof IconCmp === 'function' || IconCmp.$$typeof);
  
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        {isValidIcon && <IconCmp style={{ fontSize: 24, color: 'hsl(var(--foreground))' }} />}
        <div>
...
const SimpleCalculatorComponent: React.FC<{
  ...
  console.log(`[PERF] ========== SimpleCalculatorComponent RENDER #${renderCount.current} START ==========`);
  ...
  const resolvedCalculatorId = React.useMemo(() => {
    ...
    const queryId = new URLSearchParams(window.location.search).get('calculatorId') || undefined;
  ...
  const fetchCalculatorMetadata = React.useCallback(async (calculatorIdParam?: string) => {
let calculatorId = calculatorIdParam || config?.calculatorId;
    ...
    if (!calculatorId || calculatorId === '{calculatorId}') {
      const queryId = new URLSearchParams(window.location.search).get('calculatorId') || '';
    ...
    try {
      ...
      let urlPath = (config as any)?.dataUrl || `/api/calculators/{calculatorId}/metadata`;
...
setCalculatorMetadata(data);
  ...
  }, [config?.calculatorId]); // Add dependency for useCallback
  ...
  const { gadgetOptions, fieldList } = React.useMemo(() => {
    ...
    if (Array.isArray(config?.inputs) && (config.inputs as any[]).length > 0) {
      ...
      const withContext = optimizedFields.some((f: any) => f?.id === 'additional_context')
        ? optimizedFields
        : [
            ...optimizedFields,
            {
              ...
              placeholder: 'Enter any constraints, standards, access limitations, etc.',
              required: false,
              size: 24,
              rows: 4,
              sectionId,
      ...
      const opts = [
        { id: sectionId, type: 'section', title: 'Input Parameters', icon: 'FormOutlined', size: 24, order: 1 },
        { id: groupId, type: 'group', title: 'Details', sectionId, size: 24, order: 1, collapsible: false },
        ...withContext
      ];
    ...
    if (Array.isArray(ui)) {
      ...
      const fields = ui.filter((i: any) => i?.type && i.type !== 'section' && i.type !== 'group');

      // Ensure Additional Context exists; attach to first group or create one
      const hasAdditional = fields.some((f: any) => f?.id === 'additional_context');
      let augmented = [...ui];
      if (!hasAdditional) {
        const targetSectionId = (groups[0]?.sectionId as string) || (sections[0]?.id as string) || 'input-card';
      ...
      const optimizedAugmented = augmented.map((i: any) => {
        if ((i.type === 'textarea' || i.type === 'text') && i.type !== 'section' && i.type !== 'group') {
          return {
            ...i,
      ...
      const finalFields = optimizedAugmented.filter((i: any) => i?.type && i.type !== 'section' && i.type !== 'group');
    ...
    if (section) {
      ...
      const collected: any[] = [
        { id: sectionId, type: 'section', title: section.title || 'Input Parameters', icon: section.icon || 'FormOutlined', size: section.size || 24, order: section.order || 1 }
      ];
      const fields: any[] = [];
      if (Array.isArray(section.groups) && section.groups.length > 0) {
        section.groups.forEach((g: any, idx: number) => {
          const gid = g.id || `group-${idx + 1}`;
          collected.push({ id: gid, type: 'group', title: g.title || 'Group', sectionId, size: g.size || 24, order: g.order || (idx + 1), collapsible: !!g.collapsible });
          (g.fields || []).forEach((f: any) => {
            fields.push({ ...f, sectionId, groupId: gid, size: Number.isFinite(f?.size) ? f.size : 24 });
          });
        });
      } else if (Array.isArray(section.fields)) {
        const groupId = 'input-group';
        collected.push({ id: groupId, type: 'group', title: 'Details', sectionId, size: 24, order: 1, collapsible: false });
        section.fields.forEach((f: any) => fields.push({ ...f, sectionId, groupId, size: Number.isFinite(f?.size) ? f.size : 24 }));
      }
      // Optimize all text/textarea fields with debouncing
      ...
      const hasAdditional = optimizedFields.some((f: any) => f?.id === 'additional_context');
      ...
      return { gadgetOptions: [...collected, ...finalFields], fieldList: finalFields };
    }
  ...
  const aiPrompt = config?.aiPrompt || calculatorMetadata?.aiPrompt ||
  ...
  const markdownComponents = React.useMemo(() => ({
    h1: ({ children }: any) => <h1 style={{ color: '#111', marginBottom: 16, fontWeight: 800, letterSpacing: '-0.01em' }}>{children}</h1>,
    h2: ({ children }: any) => <h2 style={{ color: '#111', marginBottom: 12, fontWeight: 700 }}>{children}</h2>,
    h3: ({ children }: any) => <h3 style={{ color: '#111', marginBottom: 8, fontWeight: 700 }}>{children}</h3>,
    h4: ({ children }: any) => <h4 style={{ color: '#111', marginBottom: 8, fontWeight: 600 }}>{children}</h4>,
    p: ({ children }: any) => <p style={{ marginBottom: 10, lineHeight: 1.75 }}>{children}</p>,
    ul: ({ children }: any) => <ul style={{ marginBottom: 8, paddingLeft: 20 }}>{children}</ul>,
    ol: ({ children }: any) => <ol style={{ marginBottom: 8, paddingLeft: 20 }}>{children}</ol>,
    li: ({ children }: any) => <li style={{ marginBottom: 4, paddingLeft: 2 }}>{children}</li>,
    code: ({ children }: any) => (
      <code style={{
  ...
  const fieldTypeById = React.useMemo(() => {
    ...
    (fieldList || []).forEach((f: any) => { if (f?.id) map[f.id] = String(f?.type || ''); });
    return map;
  }, [fieldList]);
  ...
  const handleInputChange = React.useCallback((fieldId: string, value: any) => {
    ...
    setFormData(prev => {
      ...
      if (prev[fieldId] === value) {
        console.log(`[PERF] handleInputChange SKIP - value unchanged (${(performance.now() - startTime).toFixed(2)}ms)`);
        return prev;
      }
      console.log(`[PERF] handleInputChange UPDATE - value changed (${(performance.now() - startTime).toFixed(2)}ms)`);
      return {
        ...prev,
        [fieldId]: value
      };
    ...
    console.log(`[PERF] handleInputChange END - total: ${(endTime - startTime).toFixed(2)}ms`);
  }, []);
  ...
  const handleCalculate = React.useCallback(async () => {
    ...
    try {
      ...
      let interpolated = aiPrompt || 'Analyze the following data and provide insights:';
      Object.entries(formData).forEach(([key, value]) => {
        const sanitizedForPlaceholder = sanitizeValueForPrompt(value, fieldTypeById[key]);
        interpolated = interpolated.replace(new RegExp(`\\{${key}\\}`, 'g'), typeof sanitizedForPlaceholder === 'string' ? sanitizedForPlaceholder : JSON.stringify(sanitizedForPlaceholder));
      });
      ...
      (fieldList || []).forEach((f: any) => {
        ...
        const display = sanitized === undefined || sanitized === null || (typeof sanitized === 'string' && sanitized.trim() === '')
          ? 'N/A'
          : truncateIfLong(sanitized);
        inputLines.push(`- ${label}: ${typeof display === 'string' ? display : JSON.stringify(display)}`);
      ...
      try {
        (fieldList || []).forEach((f: any) => {
          ...
          if (Array.isArray(val)) {
            ...
            if (url.startsWith('data:image') || url.startsWith('http')) imageUrls.push(url);
          }
        });
      } catch {}
      ...
      const previousResponseId = (window as any)?.__previousResponseId || undefined;
      ...
      let finalResult = response.data || (typeof response.rawText === 'string' && response.rawText.length > 0 ? response.rawText : streamingResult) || 'No response received';
      ...
      if (!/END OF REPORT/i.test(finalResult)) {
        try {
          ...
          const continuation = await openAI.respondStream({
            ...
            promptConfig: {
              systemPrompt: 'Continue and complete the previous report. Only provide missing/remaining sections and tables. Do not repeat content. End with: END OF REPORT.',
              userPrompt: ''
            },
          ...
          if ((continuation as any)?.responseId) {
            (window as any).__previousResponseId = (continuation as any).responseId;
          }
        } catch (e) {
  ...
  const sanitizeValueForPrompt = React.useCallback((value: any, fieldType?: string): any => {
    // Nullish stays null
    ...
    if (Array.isArray(value)) {
      ...
      const looksLikeImageArray = value.length > 0 && typeof value[0] === 'object' && ('url' in (value[0] || {}) || 'base64' in (value[0] || {}));
      if (String(fieldType || '').includes('image') || looksLikeImageArray) {
        ...
        const dims = value.map((v: any) => ({ w: v?.metadata?.width, h: v?.metadata?.height })).filter((d: any) => Number.isFinite(d.w) && Number.isFinite(d.h));
        return {
          count: value.length,
      ...
      return value.slice(0, 50).map((item: any) => sanitizeValueForPrompt(item, fieldType));
    }

    // Objects: strip base64 url fields, keep metadata
    if (typeof value === 'object') {
      // Image-like object
      const hasDataUrl = typeof (value as any)?.url === 'string' && /^data:/i.test((value as any).url);
  ...
  }, [result, streamingResult, calculatorMetadata?.name, config.title, formData, fieldList]);
  ...
  const getPdfGadgetData = React.useCallback(() => ({
    reportTitle: calculatorMetadata?.name || config.title || 'Calculator Results',
  ...
  const renderPanelHeader = React.useCallback((title: string, iconName?: string) => {
    ...
    return (
      <div style={{
        ...
        {isValidIcon ? <IconCmp style={{ fontSize: 18, color: 'hsl(var(--foreground))' }} /> : null}
        <span style={{
          fontWeight: 700,
          fontSize: 14,
  ...
  const containerStyle = React.useMemo(() => ({ padding: 16 }), []);
  ...
  React.useEffect(() => {
    ...
    console.log(`[PERF] ========== SimpleCalculatorComponent RENDER #${renderCount.current} END - ${(componentRenderEnd - componentRenderStart).toFixed(2)}ms ==========`);
  });
      ...
      <div style={layoutStyle}>
          ...
          <div style={scrollableFormStyle}>
            {metadataLoading ? (
              <div style={{ padding: 20, textAlign: 'center' }}>
                <Spin size="small" />
                <div style={{ marginTop: 8 }}>
                  <Text type="secondary">Loading calculator configuration...</Text>
                </div>
              </div>
        ...
        >
          ...
          <div style={scrollableResultsStyle}>
            {isStreaming && (
              <div style={{ position: 'absolute', top: 12, right: 16, display: 'flex', alignItems: 'center', gap: 8, zIndex: 10 }}>
                <Spin size="small" />
            ...
            {loading && !streamingResult ? (
              ...
              <Text type="secondary">Results will appear here after calculation</Text>
            )}
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <Title level={4} style={{ margin: 0 }}>
                PDF Preview - {calculatorMetadata?.name || config.title || 'Calculator Results'}
              </Title>
              <Button 
                type="text" 
...
const MemoizedFormRenderer = React.memo<{
  ...
  if (!formGadgetRef.current || formGadgetRef.current.config.gadgetOptions !== gadgetOptions) {
    ...
    console.log(`[PERF] DocumentFormGadget created in ${(gadgetEndTime - gadgetStartTime).toFixed(2)}ms`);
  } else {
  ...
  const handleFormDataChange = React.useCallback((data: Record<string, any>) => {
    const startTime = performance.now();
    console.log(`[PERF] handleFormDataChange START - ${Object.keys(data).length} total fields`);
    ...
    console.log(`[PERF] handleFormDataChange - ${changedCount} changed fields detected, batching...`);
    ...
    batchTimerRef.current = setTimeout(() => {
      ...
      console.log(`[PERF] Batch processing complete in ${(batchEnd - batchStart).toFixed(2)}ms`);
    }, 0); // 0ms = next event loop (breaks out of click handler)
    
    const endTime = performance.now();
    console.log(`[PERF] handleFormDataChange END - ${(endTime - startTime).toFixed(2)}ms (async batch queued)`);
  }, [onFormDataChange]);
  ...
  console.log(`[PERF] MemoizedFormRenderer RENDER END - ${(renderEndTime - renderStartTime).toFixed(2)}ms`);
  ...
  if (prevProps.gadgetOptions.length !== nextProps.gadgetOptions.length) return false;
  ...
  const prevLast = prevProps.gadgetOptions[prevProps.gadgetOptions.length - 1]?.id;
  const nextLast = nextProps.gadgetOptions[nextProps.gadgetOptions.length - 1]?.id;
