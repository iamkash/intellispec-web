...
import {
  buildSummaryWidgets as buildGridSummaryWidgets,
  buildPromptText,
} from "../utils";

// Lazy-load heavy widgets to reduce initial bundle size
const EditableGridWidget = React.lazy(() =>
  import("../../../../widgets/input").then((m) => ({
...
export const GridSection: React.FC<GridSectionProps> = ({
    ...
    try {
      ...
      const findAnalysisIdx = () => {
        const byId = (sections || []).findIndex(
          (s) => (s as any)?.id === "image_analysis"
        );
        if (byId >= 0) return byId;
        return (wizardData.sections || []).findIndex((s) =>
          Boolean((s as any)?.imageAnalysis?.overview)
        );
      };
      ...
      const selected = (src.imageAnalysis?.selectedSuggestionIds || [])
        .map((id: string) =>
          (src.imageAnalysis?.suggestions || []).find(
            (r: any) => String(r.id) === String(id)
          )
        )
        .filter(Boolean);
      const selectedRecommendations = selected.map((r: any) => ({
        label: r.label,
        priority: r.priority,
        category: r.category,
      }));

      const promptRef: string | undefined =
        gridCfg.promptRef || gridCfg?.populate?.promptRef;
      const deep = promptRef
        ? (config as any)?.prompts?.[promptRef]
        : undefined;
      ...
      const autoGridBodyTemplate = () => {
        ...
        const generatedTemplate = [
          ...
          `Example format: [{"${cols[0]}": "Value1", "${
            cols[1] || "field2"
          }": "Value2"}, {"${cols[0]}": "Value3", "${
            cols[1] || "field2"
          }": "Value4"}]`,
        ]
          .filter(Boolean)
      ...
      const templateBodyUse =
        !template || (deep as any)?.autoPrompt === true
          ? autoGridBodyTemplate()
          : template;
      ...
      const prevId =
        (wizardData as any)?.analysisData?.previousResponseId ||
      ...
      const response = await openAI.respond({
        ...
        modelConfig: {
          ...
          maxCompletionTokens: Math.max(
            (deep?.modelConfig as any)?.maxCompletionTokens ?? 0,
            2500
          ),
        },
        promptConfig: {
      ...
      if (Array.isArray(result)) {
        ...
        if (keyFromMeta && Array.isArray((result as any)[keyFromMeta])) {
          ...
          const firstArrayKey = Object.keys(result).find((k) =>
            Array.isArray((result as any)[k])
          );
          if (firstArrayKey) {
  ...
  useEffect(() => {
    ...
    const handleAutoPopulate = (event: CustomEvent) => {
      ...
      console.log(
        `🎯 Grid section ${section.title} received auto-populate event:`,
        {
          targetSectionIndex,
      ...
      if (
        ...
        console.log(
          `⏭️ Grid section ${section.title} ignoring auto-populate event - conditions not met`
        );
      }
    };
  ...
  if (existingRows.length === 0 && (wizardData as any)?.globalFormData) {
    ...
    console.log(
      `[GridSection] Found grid data in globalFormData for ${gridCfg.dataKey}:`,
      {
  ...
  const defaultRows = (() => {
    try {
      ...
      if (!srcId) return [];
      const srcIdx = (sections || []).findIndex(
        (s) => (s as any)?.id === srcId
      );
      if (srcIdx < 0) return [];
      ...
      const picked =
        ids.length > 0
          ? ids
              .map((id: string) =>
                recs.find((r: any) => String(r.id) === String(id))
              )
              .filter(Boolean)
          : recs;
      // Flatten resources (id, name, type, quantity, notes) when present
      const rows = picked.flatMap((r: any, i: number) =>
        (Array.isArray(r.resources) ? r.resources : []).map(
          (res: any, j: number) => ({
  ...
  const rows =
    existingRows && existingRows.length > 0 ? existingRows : defaultRows;
  const setRows = (rowsNext: any[]) =>
    updateSectionData(sectionIndex, {
      formData: {
        ...((data as any).formData || {}),
        [gridCfg.dataKey]: rowsNext,
      },
    });
  ...
  const populateButton =
    gridCfg.promptRef || gridCfg?.populate?.promptRef ? (
      ...
      >
        {gridCfg.btnLabel ||
          gridCfg.populateLabel ||
          gridCfg.populate?.label ||
          "Populate from AI"}
      </Button>
    ) : null;
          ...
          <div className="analysis-stats" style={{ marginBottom: 12 }}>
            {widgets.map((w, i) => (
              <div key={i} className="stat-card">
                ...
                {w.description && (
                  <div className="stat-desc">{w.description}</div>
                )}
              </div>
            ))}
          </div>
