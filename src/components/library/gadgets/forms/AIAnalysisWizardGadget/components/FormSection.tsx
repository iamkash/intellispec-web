...
export const FormSection: React.FC<FormSectionProps> = ({
  ...
  const loadFieldOptions = useCallback(
    async (fieldId: string, field: any) => {
      if (!field.optionsUrl || loadingOptions[fieldId] || fieldOptions[fieldId])
        return;
        ...
        group.fields?.forEach((field: any) => {
          if (
            ...
            if (hasAllParentValues) {
              console.log(
                `[FormSection] Late-loading options for dependent field ${field.id} after data became available`
              );
              loadFieldOptions(field.id, field);
            }
          }
  ...
  // Dynamic field renderer using widget map - MUST be before early return
  const renderFieldWidget = useCallback(
    (field: any, value: any, updateField: (v: any) => void) => {
      const baseProps = {
      ...
      // Handle special field types with custom props
      switch (fieldType) {
        case "tel":
        case "email":
          return (
        ...
        case "text":
          return <InputFieldWidget {...baseProps} />;

        case "textarea":
          return <TextAreaWidget {...baseProps} />;
        ...
        case "date":
          return <DatePickerWidget {...baseProps} />;
        ...
        case "dropdown": {
          ...
          if (value && !options.find((opt: any) => opt.value === value)) {
            const labelMap: Record<string, string> = {
              site_id: data?.formData?.facilityName,
              asset_group_id: data?.formData?.assetGroupName,
              asset_id: data?.formData?.assetName,
              company_id: data?.formData?.companyName,
            };
            const label = labelMap[field.id] || value;
          ...
          return (
            <ComboBoxWidget
              ...
              placeholder={
                field.placeholder ||
                (loadingOptions[field.id] ? "Loading..." : "Select...")
              }
        ...
        case "multi-select":
          return <TagsInputWidget value={value || []} onChange={updateField} />;

        case "file":
          return (
            <UploadWidget
              onChange={({ fileList }: any) => updateField(fileList)}
            />
          );
    ...
    [disabledFields, fieldOptions, loadingOptions, data?.formData, user]
  );
  ...
  const handleFieldChange = (fieldId: string, newValue: any, field: any) => {
    ...
    if (field.id === "company_id") {
      const option = fieldOptions[fieldId]?.find(
      ...
      if (option?.label) {
        ...
        if (
          assetName.includes("reactor") ||
          assetName.includes("vessel") ||
          assetName.includes("tank")
        ) {
          ...
          assetName.includes("exchanger") ||
          assetName.includes("boiler")
        ) {
    ...
    section.form.groups?.forEach((group: any) => {
      group.fields?.forEach((depField: any) => {
        if (depField.dependsOn) {
          const dependencies = Array.isArray(depField.dependsOn)
            ? depField.dependsOn
            : [depField.dependsOn];
              ...
              >
                <Row gutter={[12, 12]}>
                  {(group.fields || []).map((field: any) => {
                    ...
                    // DEBUG: Parent project context loading
                    if (
                      field.populateFromAsset &&
                      process.env.NODE_ENV === "development"
                    ) {
                      console.log(`[DEBUG] Field ${field.id}:`, {
                        populateFromAsset: field.populateFromAsset,
                        currentValue: value,
                        shouldBePopulated: !!field.populateFromAsset,
                      });
                    }
                    ...
                    return (
                      ...
                      >
                        <Form.Item
                          ...
                          wrapperCol={{ span: 24 }}
                        >
                          {renderFieldWidget(field, value, updateField)}
                        </Form.Item>
                      </Col>
                    );
                  })}
