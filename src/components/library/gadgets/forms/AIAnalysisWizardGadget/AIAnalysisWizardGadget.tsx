...
export class AIAnalysisWizardGadget extends BaseGadget {
  ...
  public metadata: GadgetMetadata = {
    ...
    description:
      "Generic AI-powered domain analysis wizard with metadata-driven configuration",
    version: "1.0.0",
  ...
  private initializeConfig(props: any): void {
    ...
    if (
      ...
      Array.isArray((config as any).groups) &&
  ...
  public validate(config: GadgetConfig): ValidationResult {
    ...
    const hasSections =
      Array.isArray(this.config.steps.sections) &&
      this.config.steps.sections.length > 0;
    ...
    if (this.config.steps.input) {
      ...
      if (
        !Array.isArray(input.domainTypeOptions) ||
        input.domainTypeOptions.length === 0
      ) {
  ...
  public getStepProgress(totalStepsOverride?: number): number {
    ...
    const sectionsCount =
      this.config?.steps?.sections?.length ||
      (this.config as any)?.sections?.length ||
      0;
