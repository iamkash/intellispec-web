...
import { Alert, Badge, Button, Card, Col, Form, message, Row, Space, Spin, Steps, Tooltip, Typography } from 'antd';
...
import { createWidgetFactory, extractFieldSpecificProps, getPropsForWidget } from './widgetFactory';
...
const validateLineItems = (formData: FormData): { isValid: boolean; errors: Record<string, string> } => {
...
export const FormRenderer: React.FC<FormRendererProps> = ({ gadget, initialProps, onFormDataChange, fitContent }) => {
  const mode = (gadget.config as any).mode;
  
  // Memoize the data URL to prevent unnecessary re-fetches
  const memoizedDataUrl: string | undefined = React.useMemo(() => gadget.config.dataUrl, [gadget.config.dataUrl]);
  ...
  const [fieldConfigs, setFieldConfigs] = React.useState<Record<string, FieldConfig>>({});
  ...
  const [sectionGroups, setSectionGroups] = React.useState<Record<string, string[]>>({});
  const [groupFields, setGroupFields] = React.useState<Record<string, string[]>>({});
  const [validationResult, setValidationResult] = React.useState<FormValidationResult>({ isValid: true, errors: {}, warnings: {} });
  ...
  const [stepValidation, setStepValidation] = React.useState<Record<string, FormValidationResult>>({});
  ...
  const getFieldIcon = (fieldId: string, fieldConfigs: Record<string, FieldConfig>): React.ReactNode => {
  ...
  const loadSectionMetadata = React.useCallback(async (sectionId: string, sectionOptionsUrl: string) => {
...
setLoadingSections(prev => new Set(prev).add(sectionId));
    ...
    try {
      ...
      if (rawData.type === 'section' && rawData.groups && Array.isArray(rawData.groups)) {
        ...
        rawData.groups.forEach((group: any) => {
          ...
          if (group.watchField) {
            console.log(`[SECTION LOAD] Group ${group.id} has conditional logic:`, {
              watchField: group.watchField,
              showWhen: group.showWhen,
              showOnMatch: group.showOnMatch
            });
        ...
        rawData.forEach(item => {
          if (item.type === 'group') {
            ...
            // Debug: Log conditional properties for groups
            if (item.watchField) {
              console.log(`[SECTION LOAD] Group ${item.id} has conditional logic:`, {
                watchField: item.watchField,
                showWhen: item.showWhen,
                showOnMatch: item.showOnMatch
              });
      ...
      console.error(`[FormRenderer] Error loading section ${sectionId}:`, error);
      setSectionErrors(prev => ({
        ...prev,
        [sectionId]: error instanceof Error ? error.message : 'Failed to load section metadata'
      }));
    } finally {
  ...
  React.useEffect(() => {
    if (mode === 'wizard' && wizardSteps.length > 0) {
      ...
      if (currentStepObj) {
        ...
        if (section && section.sectionOptionsUrl && !loadedSections.has(currentStepObj.id) && !loadingSections.has(currentStepObj.id)) {
          loadSectionMetadata(currentStepObj.id, section.sectionOptionsUrl);
        }
      }
    }
  }, [mode, wizardSteps, currentStep, sections, loadedSections, loadingSections, loadSectionMetadata]);

  // Load sections when they become visible (for regular mode)
  React.useEffect(() => {
    if (mode !== 'wizard' && activeSection) {
      const section = sections[activeSection];
      if (section && section.sectionOptionsUrl && !loadedSections.has(activeSection) && !loadingSections.has(activeSection)) {
        loadSectionMetadata(activeSection, section.sectionOptionsUrl);
      }
    }
  }, [mode, activeSection, sections, loadedSections, loadingSections, loadSectionMetadata]);
  ...
  React.useEffect(() => {
    ...
    if (isInitialized && Object.keys(sections).length > 0) {
      Object.keys(sections).forEach(sectionId => {
        ...
        if (section && section.sectionOptionsUrl && !loadedSections.has(sectionId) && !loadingSections.has(sectionId)) {
          loadSectionMetadata(sectionId, section.sectionOptionsUrl);
        }
      });
    }
  }, [isInitialized, sections, loadedSections, loadingSections, loadSectionMetadata]);

  // Initialize wizard steps after sections are loaded (only once when sections change)
  React.useEffect(() => {
    if (mode === 'wizard' && isInitialized && Object.keys(sections).length > 0 && loadedSections.size > 0) {
      // Wait for at least some sections to be loaded before creating wizard steps
      const sectionsWithUrl = Object.values(sections).filter(section => section.sectionOptionsUrl);
  ...
  }, [mode, isInitialized, sections, loadedSections, gadget, groups, fieldConfigs, sectionGroups, groupFields]); // Removed formData to prevent re-initialization on field changes
  ...
  React.useEffect(() => {
    ...
    const handleScrollSpy = () => {
      ...
      Object.keys(sectionRefs.current).forEach(sectionId => {
        ...
        const visibleBottom = Math.min(sectionTop + sectionHeight, containerTop + containerHeight);
      ...
      if (bestSection && bestSection !== activeSection && bestVisibility > 0.3) {
  ...
  const widgetFactory = React.useMemo(() => createWidgetFactory(gadget), [gadget]);

  // Calculate formula value for calculated fields
  const calculateFormulaValue = React.useCallback((fieldPath: string, config: FieldConfig): number => {
    if (!config.formula) return 0;
    
    // Formula evaluation

      const result = commercialFormulaCalculator.evaluateWithContext(config.formula, { formData });
  ...
  const calculateSmartDefaults = React.useCallback((fieldPath: string, config: FieldConfig, currentFormData: FormData): any => {
    ...
    switch (fieldPath) {
      ...
      case 'lockout-tagout-required':
        ...
        if (['internal', 'combined'].includes(currentFormData['inspection-type'])) {
          return 'yes';
        }
        break;
      ...
      case 'hot-work-permit':
        ...
        if (Array.isArray(equipmentUsed) && equipmentUsed.some(eq => 
      ...
      case 'pre-inspection-approval':
        ...
        if (safetyChecks.every(check => check === 'yes' || check === 'not-required')) {
          return 'approved';
        }
        break;
      ...
      case 'ppe-available':
        ...
        if (currentFormData['ppe-required'] && Array.isArray(currentFormData['ppe-required']) && currentFormData['ppe-required'].length > 0) {
          return 'yes';
        }
        break;
      ...
      case 'electrical-equipment-safe':
        ...
        if (Array.isArray(electricalEquipment) && electricalEquipment.some(eq => 
          ['ut-gauge', 'digital-camera', 'thermal-camera', 'vibration-analyzer'].includes(eq))) {
          return 'yes';
        }
        return 'not-required';
      ...
      case 'all-safety-requirements-met':
        ...
        if (criticalSafetyChecks.every(check => check === 'yes' || check === 'not-required')) {
          return 'yes';
        }
        break;
      ...
      case 'next-inspection-interval':
        ...
        if (currentFormData['overall-assessment'] === 'acceptable') {
          return '5-years';
        } else if (currentFormData['overall-assessment'] === 'acceptable-monitoring') {
          return '2-years';
        } else if (currentFormData['overall-assessment'] === 'conditional') {
          return '1-year';
        } else if (currentFormData['overall-assessment'] === 'repair-required') {
          return 'as-required';
        }
        break;
      ...
      case 'api-510-compliance':
        // Default based on overall assessment
        if (currentFormData['overall-assessment'] === 'acceptable' || currentFormData['overall-assessment'] === 'acceptable-monitoring') {
          return 'compliant';
        } else if (currentFormData['overall-assessment'] === 'conditional') {
          return 'conditional';
        } else {
      ...
      case 'return-to-service-approved':
        ...
        if (currentFormData['overall-assessment'] === 'acceptable') {
          return 'approved';
        } else if (currentFormData['overall-assessment'] === 'acceptable-monitoring' || currentFormData['overall-assessment'] === 'conditional') {
          return 'conditional';
        } else {
          return 'not-approved';
        }
      ...
      case 'supervisor-approval-required':
        ...
        if (currentFormData['critical-findings'] === 'yes' || currentFormData['overall-assessment'] === 'repair-required' || currentFormData['overall-assessment'] === 'replacement-required') {
          return 'yes';
        }
        return 'not-applicable';
      ...
      case 'client-approval-required':
        // Default based on overall assessment
        if (currentFormData['overall-assessment'] === 'conditional' || currentFormData['overall-assessment'] === 'repair-required' || currentFormData['overall-assessment'] === 'replacement-required') {
          return 'yes';
        }
        return 'not-applicable';
      ...
      case 'report-urgency':
        ...
        if (currentFormData['overall-assessment'] === 'immediate-shutdown') {
          return 'critical';
        } else if (currentFormData['overall-assessment'] === 'repair-required' || currentFormData['overall-assessment'] === 'replacement-required') {
          return 'urgent';
        } else if (currentFormData['overall-assessment'] === 'conditional') {
          return 'standard';
        }
      ...
      case 'submission-date':
        ...
        if (currentFormData['submission-status'] === 'submitted' || currentFormData['submission-status'] === 'completed') {
          return new Date().toISOString().split('T')[0];
        }
        break;
      ...
      case 'ut-gauge-calibration':
        if (Array.isArray(currentFormData['equipment-used']) && 
      ...
      case 'measuring-tools-calibration':
        if (Array.isArray(currentFormData['equipment-used']) && 
            currentFormData['equipment-used'].some(eq => ['measuring-tape', 'caliper'].includes(eq))) {
          // Set to 6 months from today
          const date = new Date();
          date.setMonth(date.getMonth() + 6);
          return date.toISOString().split('T')[0];
  ...
  const [loadedFields, setLoadedFields] = React.useState<Set<string>>(new Set());
...
return;
    ...
    const loadIndependentFieldOptions = async () => {
      ...
      for (const [fieldId, fieldConfig] of Object.entries(fieldConfigs)) {
        ...
        if (loadedFields.has(fieldId) || loadingFields.has(fieldId) || fieldConfig.dependsOn) {
          continue;
        }

        if (fieldConfig.optionsUrl && (!fieldConfig.options || fieldConfig.options.length === 0)) {
      ...
      const loadPromises = fieldsToLoad.map(async (fieldId) => {
        ...
        try {
          const response = await BaseGadget.makeAuthenticatedFetch(fieldConfig.optionsUrl!);
          ...
          if (response.ok) {
            ...
            const options = Array.isArray(optionsData) ? optionsData.map(item => ({
              label: item[fieldConfig.optionsLabelField || 'label'] || item.name || item.label,
              value: item[fieldConfig.optionsValueField || 'value'] || item.id || item.value
            })) : [];
return { fieldId, options };
          } else {
            console.warn(`[FormRenderer] Failed to load options for ${fieldId}: ${response.status} ${response.statusText}`);
            return { fieldId, options: [] };
          }
        } catch (err) {
          console.warn(`[FormRenderer] Failed to load options for ${fieldId}:`, err);
          return { fieldId, options: [] };
        }
      });
  ...
  const [loadedDependentFields, setLoadedDependentFields] = React.useState<Set<string>>(new Set());
  ...
  React.useEffect(() => {
    ...
    const loadDependentFieldOptions = async () => {
      const fieldsToLoad: Array<{fieldId: string, optionsUrl: string, parentValue: string, loadKey: string}> = [];
      ...
      for (const [fieldId, fieldConfig] of Object.entries(fieldConfigs)) {
        ...
        if (fieldConfig.dependentOptionsUrl) {
          optionsUrl = fieldConfig.dependentOptionsUrl.replace('{parentValue}', parentValue);
        ...
        if (optionsUrl) {
          ...
          console.warn(`[FormRenderer] No optionsUrl constructed for dependent field ${fieldId}`);
        }
      }
      ...
      const loadPromises = fieldsToLoad.map(async ({ fieldId, optionsUrl, parentValue, loadKey }) => {
        const fieldConfig = fieldConfigs[fieldId];
        try {
          
          const response = await BaseGadget.makeAuthenticatedFetch(optionsUrl);
          ...
          if (response.ok) {
            ...
            console.warn(`[FormRenderer] Failed to load dependent options for ${fieldId}: ${response.status} ${response.statusText}`);
            return { fieldId, options: [], parentValue, loadKey };
          }
        } catch (err) {
          console.warn(`[FormRenderer] Failed to load dependent options for ${fieldId}:`, err);
          return { fieldId, options: [], parentValue, loadKey };
        }
      });
  ...
  React.useEffect(() => {
    const loadMockData = async () => {
          ...
          for (const [fieldId, fieldConfig] of Object.entries(fieldConfigs)) {
            if (prePopulateData[fieldId] && fieldConfig.fetchDisplayName) {
              try {
                ...
                const url = fetchConfig.url.replace('{id}', prePopulateData[fieldId]);
                ...
                if (response.ok) {
                  ...
                  const displayFieldName = fetchConfig.storeAs || `_${fieldId}_name`;
                  
                  enhancedData[displayFieldName] = displayValue;
                }
              } catch (err) {
                console.warn(`[FormRenderer] Failed to fetch ${fieldId} display name:`, err);
              }
            }
          }
      ...
      try {
        const response = await BaseGadget.makeAuthenticatedFetch(resolvedDataUrl);

        
        if (!response.ok) {
          const errorText = await response.text();
          console.error('[FormRenderer] API Error:', response.status, response.statusText, errorText);
          throw new Error(`Failed to load record data: ${response.status} ${response.statusText}`);
        }
        ...
        const properlyInitializedData = gadget.initializeFormData(apiData, fieldConfigs);
  ...
  React.useEffect(() => {
      ...
      if (Object.keys(fieldConfigs).length > 0 && memoizedDataUrl) {
        ...
        const loadMockData = async () => {
          ...
          try {
            ...
            const loadedFormData = mockData.formData || mockData.data || mockData || {};
            ...
            console.error('[FormRenderer] URL change - Error loading data:', err);
    ...
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('id') || urlParams.get('recordId') || urlParams.get('restoreId')) {
  ...
  React.useEffect(() => {
    ...
    const initialFormData = gadget.initializeFormData(initialProps.data || {}, initialFieldConfigs);
  ...
  React.useEffect(() => {
    ...
    if (mode === 'wizard' && isInitialized) {
      ...
      if (gadget.config.gadgetOptions && gadget.config.gadgetOptions.length > 0) {
        const parsedStructure = gadget.parseGadgetOptions(gadget.config.gadgetOptions);
        setSections(parsedStructure.sections);
        setGroups(parsedStructure.groups);
        setSectionGroups(parsedStructure.sectionGroups);
        setGroupFields(parsedStructure.groupFields);
  ...
  const handleNextStep = React.useCallback(() => {
    ...
    const validation = WizardUtils.validateStep(currentStepObj.id, currentStepData, wizardSteps);
    ...
    if (stepCompleted) {
      ...
      console.warn('FormRenderer: Gadget step completion failed, using fallback');
  ...
  const handleStepChange = React.useCallback((stepIndex: number) => {
    ...
    if (stepIndex <= currentStep || completedSteps.has(stepIndex)) {
      ...
      message.error('Cannot navigate to this step. Please complete the current step first.');
    }
      ...
      if (Object.keys(fieldConfigs).length > 0) {
        ...
        Object.entries(fieldConfigs).forEach(([fieldPath, config]) => {
          if (config.calculated && config.formula) {
            ...
            const result = commercialFormulaCalculator.evaluateWithContext(config.formula, { formData });
            const calculatedValue = typeof result.value === 'number' ? result.value : 
                                  typeof result.value === 'string' ? parseFloat(result.value) || 0 : 0;
  ...
  const handleFieldChange = React.useCallback((fieldPath: string, value: any) => {
  ...
  const getSortedSections = (): FormSection[] => {
    return Object.values(sections).sort((a, b) => (a.order || 0) - (b.order || 0));
  };
  ...
  const getSortedGroups = (sectionId: string): FormGroup[] => {
    ...
    const visibleGroups = sortedGroups.filter(group => {
      ...
      if (group.watchField && group.showWhen !== undefined) {
        ...
        if (typeof group.showWhen === 'string' && group.showWhen.includes(',')) {
          const showWhenArray = group.showWhen.split(',').map((s: string) => s.trim());
    ...
    if (sectionId === 'safety-preparation' || sectionId === 'visual-inspection') {
  ...
  const validateField = React.useCallback((fieldPath: string, config: FieldConfig, value: any): string | null => {
    // Required field validation
    if (config.required && (value === undefined || value === null || value === '' || (Array.isArray(value) && value.length === 0))) {
  ...
  const shouldShowField = React.useCallback((config: FieldConfig, formData: FormData): boolean => {
    ...
    if (typeof config.showWhen === 'string' && config.showWhen.includes(',')) {
  ...
  const getSectionErrorCount = React.useCallback((sectionId: string): number => {
    ...
    return errorCount;
  }, [validationResult.errors, getSortedGroups, getFieldsForGroup, fieldConfigs, formData, shouldShowField]);
  ...
  const validateConditionalFields = React.useCallback((watchedFieldPath: string, watchedValue: any) => {
    // Get only the fields that depend on this field (much more efficient)
    const dependentFields = conditionalFieldDependencies[watchedFieldPath] || [];
    
    if (dependentFields.length === 0) return;

    const fieldsToValidate: Array<{ fieldPath: string; config: FieldConfig; value: any }> = [];
  ...
  }, [conditionalFieldDependencies, fieldConfigs, formData, validateField, shouldShowField]);

  // Handle field changes for wizard mode - optimized with refs
  const handleWizardFieldChange = React.useCallback((fieldPath: string, value: any) => {
    if (modeRef.current !== 'wizard' || wizardStepsRef.current.length === 0) return;
    ...
    if (config) {
      ...
      setStepValidation(prev => {
        const currentStepValidation = prev[currentStepObj.id] || { isValid: true, errors: {}, warnings: {} };
  ...
  const performValidation = React.useCallback((fieldPath: string, value: any) => {
    const config = fieldConfigs[fieldPath];
    if (!config) return;
  ...
  const handleFieldChangeWithValidation = React.useCallback((fieldPath: string, value: any) => {
  ...
  const renderField = (fieldPath: string, config: FieldConfig): React.ReactNode => {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const isPrePopulated = config.disableWhenPrePopulated && urlParams.get(config.id);
    ...
    if (isPrePopulated && config.prePopulatedFieldType) {
      ...
      if (displayNameField && formData[displayNameField]) {
        ...
        const selectedOption = config.options.find(opt => opt.value === effectiveValue);
        if (selectedOption) {
    ...
    const baseProps = {
      ...
      onChange: (newValue: any) => handleFieldChangeWithValidation(fieldPath, newValue),
    ...
    const widgetFactoryFn = widgetFactory[effectiveFieldType as keyof typeof widgetFactory];
    
    if (widgetFactoryFn) {
      const props = getPropsForWidget(effectiveFieldType || '', baseProps, underlinedProps, standardProps);
      return widgetFactoryFn(props);
    }
    
    // Fallback to default widget
    console.warn(`Widget type "${effectiveFieldType}" not found in factory, using default InputFieldWidget`);
    return React.createElement(require('../../widgets/input').InputFieldWidget, underlinedProps);
  };

    const renderGroupCard = (group: FormGroup): React.ReactNode => {
    const isCollapsed = collapsedGroups[group.id] || (group.defaultCollapsed && collapsedGroups[group.id] === undefined);
    const fields = getFieldsForGroup(group.id);
        ...
        {!isCollapsed && (
          ...
          >
            {fields.map(fieldPath => {
              ...
              const baseSpan = config.size ? Math.min(Number(config.size), 24) : 24;
              ...
              return (
                <Col key={fieldPath} {...responsiveSpan} style={{ display: 'flex', flexDirection: 'column' }}>
                  <Form.Item
                    key={fieldPath}
                    label={
                      config.tooltip ? (
                        <Space size="small">
                          ...
                          <Tooltip title={config.tooltip}>
                            <QuestionCircleOutlined style={{ color: 'var(--color-text-secondary)', fontSize: '12px' }} />
                          </Tooltip>
                        </Space>
                      ) : (
                        effectiveLabel
  ...
  const renderSectionContent = (sectionId: string): React.ReactNode => {
    ...
    if (isLoading) {
      return (
        <div style={{ textAlign: 'center', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
    ...
    if (groups.length === 0) {
      return (
        ...
        <div className="form-section-content">
          <Form layout="vertical" className="form-underline" style={{ height: '100%' }}>
          <Row gutter={[16, 24]}>
          {groups.map(group => {
            // Use group.size directly for 24-column grid system
            const baseSpan = group.size ? Math.min(Number(group.size), 24) : 24;
            const responsiveSpan = {
              xs: baseSpan, // Use defined size on all screen sizes
            ...
            return (
              <Col key={group.id} {...responsiveSpan} style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div
              ...
              >
                <div style={{ position: 'relative', marginRight: '12px', flexShrink: 0 }}>
                  ...
                  {hasSectionError && (
                    <Badge 
                      count={<AntIcons.ExclamationCircleOutlined style={{ color: 'hsl(var(--destructive-foreground))', fontSize: '8px' }} />}
                      size="small" 
                      style={{ 
                ...
                {hasErrors && (
                  <div style={{ 
                    fontSize: '11px', 
                    color: isSelected ? 'hsl(var(--primary-foreground))' : 'hsl(var(--destructive))',
                    marginLeft: '8px',
                    flexShrink: 0
                  }}>
                    {errorCount}
  ...
  const renderFormHeader = (): React.ReactNode => {
    ...
    const handleClose = () => {
      ...
      const getIntelligentFallback = (): string | null => {
        ...
        console.log('[FormRenderer] Determining fallback for workspace:', currentWorkspace);
        ...
        if (workspaceParts.length >= 2) {
          ...
          if (formName.endsWith('-document') || formName.endsWith('-form')) {
            const baseName = formName.replace('-document', '').replace('-form', '');
            fallbackWorkspace = `${baseModule}/${baseName}-list`;
          } else if (formName.endsWith('-edit') || formName.endsWith('-create')) {
            const baseName = formName.replace('-edit', '').replace('-create', '');
            fallbackWorkspace = `${baseModule}/${baseName}-list`;
          }
          ...
          console.log('[FormRenderer] Fallback workspace determined:', fallbackWorkspace);
      ...
      if (navigationSource) {
        try {
          ...
          if (sourceWorkspace && sourceWorkspace !== new URL(currentUrl).searchParams.get('workspace')) {
            targetUrl = `${window.location.origin}/?workspace=${encodeURIComponent(sourceWorkspace)}`;
            console.log('[FormRenderer] Using sessionStorage navigation source:', sourceWorkspace);
      ...
      if (!targetUrl) {
        ...
        if (fallbackWorkspace) {
          targetUrl = `${window.location.origin}/?workspace=${encodeURIComponent(fallbackWorkspace)}`;
          console.log('[FormRenderer] Using intelligent fallback:', fallbackWorkspace);
        }
      }
      ...
      if (config.onCancel?.action === 'navigate' && config.onCancel?.target) {
        if (config.onCancel.target === 'back') {
          ...
          const specificUrl = `${window.location.origin}/?workspace=${encodeURIComponent(config.onCancel.target)}`;
          console.log(`[FormRenderer] Navigating to specific target: ${specificUrl}`);
    ...
    return (
      <>
        <div style={{ flex: 1 }}>
          ...
          {description && (
                            <Typography.Text type="secondary" style={{ display: 'block', marginTop: 4, color: 'var(--color-text-secondary)' }}>
              {description}
            </Typography.Text>
          )}
        </div>
        {headerActions && (headerActions.showClose || headerActions.showMaximize) && (
  ...
  const renderActions = (): React.ReactNode => {
    ...
    if (mode === 'wizard') {
      ...
      const currentStepObj = wizardSteps[currentStep];
      const currentStepValidation = currentStepObj ? stepValidation[currentStepObj.id] : null;
    ...
    if (!showSaveButton && !showResetButton && !showClearButton && !showCancelButton) {
      return null;
    }
              ...
              if (config.onCancel?.action === 'navigate' && config.onCancel?.target) {
                if (config.onCancel.target === 'back') {
                  if (referrerUrl && 
                    ...
                    const fallbackWorkspace = getIntelligentFallback();
                    if (fallbackWorkspace) {
                      const fallbackUrl = `${window.location.origin}/?workspace=${encodeURIComponent(fallbackWorkspace)}`;
                      console.log('[FormRenderer] Using intelligent fallback:', fallbackUrl);
                      window.location.replace(fallbackUrl);
                    } else {
                      console.log('[FormRenderer] No fallback found, using history.back()');
                      window.history.back();
                    }
                  }
                } else {
                  const targetUrl = `${window.location.origin}/?workspace=${encodeURIComponent(config.onCancel.target)}`;
                  window.location.replace(targetUrl);
                }
              } else if (config.onCancel?.action === 'close') {
                if (referrerUrl && 
            ...
            onClick={async () => {
              ...
              if (validationResult.isValid && lineItemsValidation.isValid) {
                ...
                try {
                  ...
                  const method = isEdit ? (config.updateMethod || 'PUT') : (config.submitMethod || 'POST');
                  ...
                  if (!response.ok) {
                    ...
                    throw new Error(`Save failed (${response.status}): ${errorDetails.error || errorDetails.message || response.statusText}`);
                  }
                  ...
                  const successMessage = config.onSaveSuccess?.message || 'Saved successfully!';
                  message.success(successMessage);
                  
                  // Handle navigation using window.location.replace for reliable re-render
                  if (config.onSaveSuccess?.action === 'navigate' && config.onSaveSuccess?.target) {
                    const currentUrl = window.location.href;
                    const referrerUrl = document.referrer;
                    
                    console.log('[FormRenderer] Save success - navigating to:', config.onSaveSuccess.target, {
                      currentUrl,
                      referrerUrl
                    });
                    ...
                    if (config.onSaveSuccess.target === 'back') {
                      ...
                      setTimeout(() => {
                        ...
                        if (referrerUrl && 
                            referrerUrl.includes(window.location.origin) && 
                            referrerUrl !== currentUrl) {
                          console.log('[FormRenderer] Using valid referrer after save');
                          window.location.replace(referrerUrl);
                        } else {
                          console.log('[FormRenderer] Invalid/same referrer, using history.back()');
                          window.history.back();
                        }
                      }, 500);
                    } else {
                      ...
                      setTimeout(() => {
                        const targetUrl = `${window.location.origin}/?workspace=${encodeURIComponent(config.onSaveSuccess.target)}`;
                        console.log(`[FormRenderer] Navigating to: ${targetUrl}`);
                        window.location.replace(targetUrl);
                      }, 500);
                    }
                  }
                  ...
                  if (error.response?.data?.error === 'Validation failed' && error.response?.data?.details) {
                    ...
                    const errorMessage = (gadget.config as any).onSaveError?.message || 'Failed to save. Please try again.';
  ...
  if (!isInitialized || !fieldConfigs || Object.keys(fieldConfigs).length === 0) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
        <Spin size="large" />
  ...
  return (
    <div style={{ 
      height: fitContent ? 'auto' : ((gadget.config as any).fullHeight ? 'calc(100vh - 180px)' : 'calc(100vh - 280px)'), 
      display: 'flex', 
      flexDirection: 'column',
              ...
              <React.Fragment key={index}>
                <div 
                  style={{
                    ...
                    padding: '4px 8px',
                    borderRadius: '4px',
                    cursor: gadget.config.allowStepNavigation ? 'pointer' : 'default',
                    background: isActive ? 'var(--color-primary)' : isCompleted ? 'var(--color-success-bg)' : 'transparent',
                    color: isActive ? 'var(--color-text)' : isCompleted ? 'var(--color-success)' : 'var(--color-text-secondary)',
                    fontWeight: isActive ? '600' : '400',
                    fontSize: '11px',
                    transition: 'all 0.2s ease'
                  }}
                  onClick={() => gadget.config.allowStepNavigation && handleStepChange(index)}
                >
                  {step.icon && (() => {
                    try {
                      ...
                      if (IconComponent && typeof IconComponent === 'function') {
                        ...
                        return <AntIcons.FormOutlined style={{ fontSize: '10px' }} />;
                      }
                    } catch (error) {
                      return React.createElement(AntIcons.FormOutlined, { style: { fontSize: '10px' } });
                    }
                  })()}
                  <span>{step.title}</span>
                  {hasErrors && (
                    <AntIcons.ExclamationCircleOutlined style={{ 
      ...
      <div style={{ 
        ...
        {Object.keys(sections).length > 0 ? (
          <>
            ...
            {!isMobile && mode !== 'wizard' && !(gadget.config as any)?.hideSidebar && (
              <div 
              ...
              >
                <div style={{ 
                  ...
                  <Text strong style={{ color: 'hsl(var(--foreground))', fontSize: '14px', fontWeight: 600 }}>Form Sections</Text>
                </div>
            ...
            >
              {mode === 'wizard' ? (
                ...
                (() => {
                  ...
                  const currentSection = sortedSections.find(section => section.id === currentStepObj.id);
                  if (!currentSection) {
                    return (
                      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
                  ...
                  return (
                    <div style={{ padding: 0, borderRadius: 0, background: 'transparent', border: 'none' }}>
                ...
                <div>
                  {sortedSections.map(section => (
                    ...
                    >
                      ...
                      >
                        {getSectionIcon(section.id, sections)}
                        <Title level={5} style={{ color: 'inherit', fontSize: '14px', fontWeight: 600 }}>
                          {section.title}
                        </Title>
                        {getSectionErrorCount(section.id) > 0 && (
                          <Badge count={getSectionErrorCount(section.id)} size="small" style={{ backgroundColor: 'var(--color-error)' }} />
                        )}
                      </div>
                  ...
                  return (
                    <Row gutter={[16, 24]}>
                      {sectionGroups.map(group => {
                        ...
                        const wizardFields = groupFieldIds.filter(fieldId => 
                          currentStepObj.fields[fieldId]
                        );
      ...
      {(gadget.config as any).aiAssistant?.enabled && (
        <AIChatbotWidget
          ...
          description={(gadget.config as any).aiAssistant?.description || 'Get help with form filling and inspection documentation'}
          ...
          openaiConfig={{
            ...
            apiKey: process.env.REACT_APP_OPENAI_API_KEY || (gadget.config as any).aiAssistant?.openaiConfig?.apiKey
          }}
          modelConfig={(gadget.config as any).aiAssistant?.modelConfig}
          enableVoice={(gadget.config as any).aiAssistant?.features?.enableVoice}
          enableImageUpload={(gadget.config as any).aiAssistant?.features?.enableImageUpload}
          enableDocumentUpload={(gadget.config as any).aiAssistant?.features?.enableDocumentUpload}
          enableSuggestions={(gadget.config as any).aiAssistant?.features?.enableSuggestions}
          enableAutoFill={(gadget.config as any).aiAssistant?.features?.enableAutoFill}
          ...
          onSectionChange={(sectionId) => {
            ...
            if (mode === 'wizard' && wizardSteps.length > 0) {
              const stepIndex = wizardSteps.findIndex(step => 
                Object.keys(step.fields).some((field: string) => fieldConfigs[field]?.sectionId === sectionId)
              );
              if (stepIndex !== -1) {
                setCurrentStep(stepIndex);
              }
