 ...
 */
export const EditableGridWidget: React.FC<EditableGridWidgetProps> = ({
  rows,
  onChange,
  columnsMeta,
  features,
  ...
  const [pagination, setPagination] = React.useState({
    pageIndex: 0,
    pageSize: features?.pageSizeOptions?.[0] || 5,
  });
    ...
    const fetchOptionsForColumn = async (col: EditableGridColumnMeta) => {
      if (col.type === "select" && col.optionsUrl) {
        ...
        if (columnOptions[col.key] && columnOptions[col.key].length > 0) {
          console.log(
            `[EditableGridWidget] Options already loaded for ${col.key}:`,
            columnOptions[col.key].length
          );
          return;
        }
        ...
        try {
          ...
          console.log(
            `[EditableGridWidget] Original URL for ${col.key}:`,
            optionsUrl
          );
          ...
          console.log(
            `[EditableGridWidget] Resolved URL for ${col.key}:`,
            optionsUrl
          );
          ...
          if (!response.ok) {
            console.warn(
              `[EditableGridWidget] Failed to fetch options for ${col.key}:`,
              response.status
            );
          ...
          console.log(
            `[EditableGridWidget] Raw API response for ${col.key}:`,
            result
          );
          ...
          console.log(
            `[EditableGridWidget] Parsed options for ${col.key} (before mapping):`,
            options
          );
            ...
            options = options.map((opt) => {
              ...
              return {
                ...
                label:
                  optAny[labelField] ||
                  opt.label ||
                  String(optAny[valueField] || opt.value || ""),
          ...
          console.log(
            `[EditableGridWidget] Final options for ${col.key}:`,
            options
          );
          setColumnOptions((prev) => ({ ...prev, [col.key]: options }));
        } catch (err) {
          console.error(
            `[EditableGridWidget] Error fetching options for ${col.key}:`,
            err
          );
          setColumnOptions((prev) => ({ ...prev, [col.key]: [] }));
        } finally {
  ...
  const addRow = React.useCallback(() => {
    ...
    (columnMeta || []).forEach((cm) => {
      if (!(cm.key in newRow)) (newRow as any)[cm.key] = "";
    });
  ...
  const update = React.useCallback(
    (id: string, patch: Partial<EditableGridRow>) => {
      isInternalUpdate.current = true;
      setData((prev) =>
  ...
  const columns = React.useMemo<ColumnDef<EditableGridRow>[]>(() => {
    const selectionCol: ColumnDef<EditableGridRow> | null =
        ...
        ({
          ...
          cell: (info: any) => {
            ...
            if (cm.type === "select") {
              ...
              console.log(
                `[EditableGridWidget] Rendering select for ${cm.key} in edit mode:`,
                {
              ...
              const handleSelectChange = (v: any) => {
                ...
                if (cm.autoPopulate) {
                  ...
                  if (selectedOption) {
                    ...
                    Object.entries(cm.autoPopulate).forEach(
    ...
    const actionsCol: ColumnDef<EditableGridRow> = {
      ...
      cell: (info: any) => {
        ...
        return isEditing ? (
          <Space>
            <Button type="primary" size="small" onClick={commitEdit}>
              Save
            </Button>
          ...
          <Space>
            <Button size="small" onClick={() => startEdit(row.id)}>
              Edit
            </Button>
            <Button danger size="small" onClick={() => remove(row.id)}>
              Delete
            </Button>
          </Space>
        );
  ...
  const table = useReactTable({
    ...
    onGlobalFilterChange:
      features?.enableFiltering === false
        ? undefined
        : (setGlobalFilter as any),
    onPaginationChange:
      features?.enablePagination === false ? undefined : (setPagination as any),
  ...
  const tableContainerStyle: React.CSSProperties = {
    overflowX: "auto",
    border:
      features?.bordered === false ? "none" : "1px solid hsl(var(--border))",
    borderTop: "none",
    borderRadius: 8,
        ...
        <Space>
          {features?.enablePagination !== false && (
            <Select
              ...
              onChange={(v) =>
                setPagination((p) => ({ ...p, pageSize: v as number }))
              }
              style={{ width: 120 }}
              options={(features?.pageSizeOptions || [5, 10, 20, 50]).map(
                (n) => ({ label: `Rows: ${n}`, value: n })
              )}
            />
          )}
          {!readOnly && (
            <>
              ...
              <Button
                size="small"
                onClick={() => {
                  const one = table.getSelectedRowModel().rows[0];
                  if (one) startEdit(one.original.id);
                }}
              ...
              <tr key={hg.id}>
                {hg.headers.map((h) => (
                  <th
                    ...
                    className="meta-th"
                    style={{
                      ...thStyle((h as any).getSize?.() as any),
                      cursor:
                        features?.enableSorting !== false &&
                        h.column.getCanSort()
                          ? "pointer"
                          : "default",
                    }}
          ...
          <tbody>
            {table.getRowModel().rows.map((r, idx) => (
              <tr
                ...
                style={{
                  background:
                    idx % 2 === 1 ? "hsl(var(--muted) / 0.25)" : undefined,
                }}
              >
                {r.getVisibleCells().map((c) => (
                  <td key={c.id} className="meta-td" style={tdStyle}>
